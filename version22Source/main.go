...
type Message struct {
	...
	MsgType    string            `json:"msg_type"`             // TEXT, FILE_TICKET, TOMBSTONE, PRESENCE, CHAT_REQUEST, VOICE_JOIN, VOICE_LEAVE, VOICE_PING, WEBRTC_OFFER, WEBRTC_ANSWER, WEBRTC_ICE, PLATFORM_BAN, PLATFORM_UNBAN
...
func main() {
	portFlag := flag.String("port", "0", "Port to run the Aegis node on (0 automatically finds a free port)")
	...
	currentPort = fmt.Sprintf("%d", listener.Addr().(*net.TCPAddr).Port)
	...
	http.HandleFunc("/api/reset", apiResetNode)
	...
	http.HandleFunc("/api/messages/ack", apiAckMessage) 
	...
	http.HandleFunc("/p2p/files/push", p2pReceiveFile) 
...
func apiBanUser(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		// Broadcast decentralized ban event
		node.Clock.Increment()
		msgType := "PLATFORM_BAN"
		if req.Action == "unban" {
			msgType = "PLATFORM_UNBAN"
		}
		...
		payloadToSign := fmt.Sprintf("%s:%s:%s:%d", banMsg.ID, banMsg.Platform, banMsg.Text, banMsg.Timestamp.UnixNano())
...
func p2pReceiveMessage(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		// Process Decentralized Ban Events immediately
		if msg.MsgType == "PLATFORM_BAN" || msg.MsgType == "PLATFORM_UNBAN" {
			if plat, ok := node.Platforms[msg.Platform]; ok {
				if role, roleExists := plat.Members[msg.Sender]; roleExists && (role == RoleOwner || role == RoleAdmin) {
					found := -1
					for i, h := range plat.BannedHashes {
						if h == msg.Text {
							found = i
					...
					if msg.MsgType == "PLATFORM_BAN" && found < 0 {
						plat.BannedHashes = append(plat.BannedHashes, msg.Text)
					} else if msg.MsgType == "PLATFORM_UNBAN" && found >= 0 {
						plat.BannedHashes = append(plat.BannedHashes[:found], plat.BannedHashes[found+1:]...)
					}
					node.Platforms[msg.Platform] = plat
				}
			...
			go func(pURL string) {
				...
				if err == nil && resp.StatusCode == http.StatusOK {
					...
					if err := json.NewDecoder(limitReader).Decode(&msgs); err == nil {
						...
						for _, m := range msgs {
							node.mu.RLock()
							inPlatform := myPlatforms[m.Platform]
							node.mu.RUnlock()
						...
						if len(validMsgs) > 0 {
							...
							for _, m := range validMsgs {
								...
								if m.MsgType == "PLATFORM_BAN" || m.MsgType == "PLATFORM_UNBAN" {
									if plat, ok := node.Platforms[m.Platform]; ok {
										if role, roleExists := plat.Members[m.Sender]; roleExists && (role == RoleOwner || role == RoleAdmin) {
											found := -1
											for i, h := range plat.BannedHashes {
												if h == m.Text {
													found = i
											...
											if m.MsgType == "PLATFORM_BAN" && found < 0 {
												plat.BannedHashes = append(plat.BannedHashes, m.Text)
											} else if m.MsgType == "PLATFORM_UNBAN" && found >= 0 {
												plat.BannedHashes = append(plat.BannedHashes[:found], plat.BannedHashes[found+1:]...)
											}
											node.Platforms[m.Platform] = plat
											changed = true
...
<body class="h-screen flex flex-col mesh-bg" onclick="closeMemberMenu()">
    ...
    <header class="bg-slate-900 border-b border-slate-700 p-4 flex justify-between items-center shadow-lg z-10">
        <div class="flex items-center space-x-3">
            ...
            <h1 class="text-xl font-bold tracking-wider">AEGIS <span class="text-xs text-indigo-400 align-top">v2.3-VOIP</span></h1>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-700 p-6 rounded-xl w-[28rem] shadow-2xl">
            ...
            <div class="flex justify-end space-x-3 mt-6">
                ...
                <button onclick="copyInviteCode()" class="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded transition-colors flex items-center">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 00-2-2h-8a2 2 0 00-2-2v8a2 2 0 002 2z"></path></svg>
                    Copy Code
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        async function loadMessages(force = false) {
            ...
            try {
                ...
                if (msgs) {
                    ...
                    msgs.forEach(m => {
                        // Check bans and blocks EARLY
                        const isBlocked = myBlockedHashes.includes(m.sender);
                        const plat = platformsCache[m.platform];
                        const isBanned = plat && plat.banned_hashes && plat.banned_hashes.includes(m.sender);
                        
                        if (isBlocked || isBanned) {
                            processedSignals.add(m.id);
                            return; // ABORT processing completely
                        }
                        ...
                        if (!processedSignals.has(m.id)) {
							...
							if (m.msg_type === 'PLATFORM_BAN' || m.msg_type === 'PLATFORM_UNBAN') {
                                loadPlatforms(); // silently sync
                                if (m.msg_type === 'PLATFORM_BAN' && m.text === myId && m.platform === currentPlatform) {
                                    showToast("You have been banned from this platform.", "error");
                ...
                if (!userSwitchedTabs) {
                    ...
                    if(platformMsgs.length === 0) {
                        ...
                        platformMsgs.forEach(msg => {
                            if (myBlockedHashes.includes(msg.sender) || (platData && platData.banned_hashes && platData.banned_hashes.includes(msg.sender))) return;
                            if (['PRESENCE', 'CHAT_REQUEST', 'PLATFORM_BAN', 'PLATFORM_UNBAN', 'VOICE_JOIN', 'VOICE_LEAVE', 'VOICE_PING', 'WEBRTC_OFFER', 'WEBRTC_ANSWER', 'WEBRTC_ICE'].includes(msg.msg_type)) return;
        ...
        function buildMemberRow(m) {
            ...
            return '<div onclick="showMemberMenu(event, \''+escapeHTML(m.hash)+'\', \''+escapeHTML(m.name)+'\')" class="flex items-center cursor-pointer hover:bg-slate-800 p-1.5 rounded transition-colors ' + blockStyle + '">' +
                ...
                '</div>' +
                '<div class="flex-1 overflow-hidden">' +
                    '<div class="truncate text-sm text-slate-200 font-medium">' + escapeHTML(m.name) + roleBadge + voiceIcon + '</div>' +
		...
		function requestChat(targetHash, targetName) {
			...
			apiCall('/api/platforms', 'POST', { name: platName, is_ephemeral: false, ttl_hours: 0 })
			.then(p => {
		...
		function processNextChatRequest() {
			...
			document.getElementById('btnChatAccept').onclick = () => {
				localStorage.setItem('handled_req_' + req.msgId, '1');
				joinPlatformCode(req.inviteCode);
				closeChatRequestModal();
			};
        ...
        function banFromPlatform(hash) {
            ...
            showConfirm("Ban User", "Are you sure you want to permanently ban this cryptographic hash from the platform?", (confirmed) => {
                if(!confirmed) return;
                apiCall('/api/platforms/ban', 'POST', {platform_id: currentPlatform, hash: hash, action: 'ban'})
                .then(() => {
        ...
        function unbanUser(hash) {
            apiCall('/api/platforms/ban', 'POST', {platform_id: currentPlatform, hash: hash, action: 'unban'})
            .then(() => {
        ...
        function createPlatform() {
            ...
            apiCall('/api/platforms', 'POST', { name: name, is_ephemeral: false, ttl_hours: 0 })
            .then(() => {
                hideCreateModal();
