...
type Message struct {
	...
	MsgType    string            `json:"msg_type"`             // TEXT, FILE_TICKET, TOMBSTONE, PRESENCE, POLL, VOTE, SUB_PLATFORM, VOICE_JOIN, VOICE_LEAVE, WEBRTC_OFFER, WEBRTC_ANSWER, WEBRTC_ICE
	TargetMsg  string            `json:"target_msg,omitempty"` // For linking Votes to Polls
	TargetHash string            `json:"target_hash,omitempty"`// For nominating a user / WebRTC targeting
...
<body class="h-screen flex flex-col mesh-bg" onclick="closeMemberMenu()">
    ...
    <!-- Hidden audio elements injection for VOIP -->
    ...
    <header class="bg-slate-900 border-b border-slate-700 p-4 flex justify-between items-center shadow-lg z-10">
        <div class="flex items-center space-x-3">
            ...
            <h1 class="text-xl font-bold tracking-wider">AEGIS <span class="text-xs text-indigo-400 align-top">v2.1-VOIP</span></h1>
        </div>
        ...
        <main class="flex-1 flex flex-col bg-slate-900/60 backdrop-blur-sm relative">
            <div class="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/90">
                ...
                <div class="flex space-x-2 hidden" id="platformActions">
                    <button onclick="toggleVoice()" id="voiceBtn" class="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded border border-slate-700 transition-colors flex items-center shadow-sm font-bold">
                        <span class="mr-1">🎙️ Join Voice</span>
                    </button>
    ...
    <script>
        ...
        let rtcPeers = {}; // map of remoteHash -> RTCPeerConnection
        ...
        function selectPlatform(id) {
            if (voiceActive && currentPlatform !== id) {
                toggleVoice(); // Disconnect from voice when switching platforms
            }
        ...
        async function toggleVoice() {
            ...
            if (voiceActive) {
                ...
                try {
                    ...
                    showToast("Joined Encrypted Voice Channel", "success");
                    sendSignal('VOICE_JOIN', '');
                    renderMembers();
                } catch(e) {
                    showToast("Microphone access denied or unavailable. WebRTC requires localhost or HTTPS.", "error");
                }
            }
        }

        function sendSignal(type, targetHash, payloadObj = '') {
            fetch('/api/messages', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + API_TOKEN },
                body: JSON.stringify({
        ...
        function getOrCreatePeer(targetHash) {
            ...
            pc.ontrack = e => {
                let audio = document.getElementById('audio_' + targetHash);
                if (!audio) {
                    audio = document.createElement('audio');
                    audio.id = 'audio_' + targetHash;
                    audio.autoplay = true;
            ...
            pc.onconnectionstatechange = () => {
                if (pc.connectionState === 'disconnected' || pc.connectionState === 'failed' || pc.connectionState === 'closed') {
                    const audio = document.getElementById('audio_' + targetHash);
        ...
        async function handleAnswer(targetHash, answer) {
            if (rtcPeers[targetHash]) {
                await rtcPeers[targetHash].setRemoteDescription(new RTCSessionDescription(answer));
            }
        }

        async function handleIce(targetHash, ice) {
            if (rtcPeers[targetHash]) {
                try { await rtcPeers[targetHash].addIceCandidate(new RTCIceCandidate(ice)); } catch(e) {}
            }
        }
        ...
        function loadMessages() {
            ...
            fetch('/api/messages?platform=' + currentPlatform, {headers: {'Authorization': 'Bearer ' + API_TOKEN}}).then(r => r.json()).then(msgs => {
                ...
                msgs.forEach(m => {
                    ...
                    // Process WebRTC Signaling logic
                    if (!processedSignals.has(m.id)) {
                        processedSignals.add(m.id);
                        if (m.platform === currentPlatform && m.sender !== myId) {
                            if (m.msg_type === 'VOICE_JOIN' && voiceActive) {
                                // Deterministic Initiator: Higher hash creates the WebRTC offer
                                ...
                                if (rtcPeers[m.sender]) {
                                    ...
                                    const audio = document.getElementById('audio_' + m.sender);
                                    if (audio) audio.remove();
                                }
                                renderMembers();
                            } else if (m.target_hash === myId && voiceActive) {
                ...
                if(!msgs || msgs.length === 0) {
                    ...
                    msgs.forEach(msg => {
                        ...
                        // Hide underlying VOIP Signaling packets from UI completely
                        if (['VOICE_JOIN', 'VOICE_LEAVE', 'WEBRTC_OFFER', 'WEBRTC_ANSWER', 'WEBRTC_ICE'].includes(msg.msg_type)) return;
        ...
        function buildMemberRow(m) {
            ...
            // Active Voice logic display mapping
            const isVoice = (m.hash === myId && voiceActive) || !!rtcPeers[m.hash];
