...
// Vulnerability 6 Fix: Enforce structured SSRF validation, strictly blocking Metadata and unverified Loopback critical ports.
...
func registerPeer(rawURL string, remoteAddr string, doReverse bool) {
	...
	if err == nil && len(ips) > 0 {
		...
		if ips[0].IsLoopback() || ips[0].IsPrivate() {
			port := parsed.Port()
			if port == "80" || port == "443" || port == "22" || port == "6379" || port == "3306" || port == "5432" {
				fmt.Println("[P2P] Blocked SSRF attempt against internal critical service port")
				return
			}
		}
	}
